"""
WSL Guardian Service - FIXED VERSION 34
...
UNAUTHORIZED_COMMANDS = [
    ...
    {"name": "netcat", "pattern": r"\bnetcat\b"}, #terminates whenever the command line detect netcat as a whole
    {"name": "curl", "pattern": r"\bcurl\b"},
    {"name": "wget", "pattern": r"\bwget\b"},
    {"name": "tcpdump", "pattern": r"\btcpdump\b"}, #packet spamming is terminated
...
class WSLGuardian:
    ...
    def deploy_wsl_agent(self):
        """Deploys and launches a live monitoring agent inside WSL dynamically"""
        try:
            logger.info("Deploying WSL child process monitoring agent...")
            # Step 1: Write agent script line-by-line
            lines = [
                "echo '#!/bin/bash' > ~/wsl_guardian_agent.sh",
                "echo 'log_path=\"/mnt/c/ProgramData/WSLGuardian/wsl_shell_trace.log\"' >> ~/wsl_guardian_agent.sh",
                "echo 'mkdir -p /mnt/c/ProgramData/WSLGuardian' >> ~/wsl_guardian_agent.sh",
                "echo '[ -f /tmp/wsl_guardian_active ] && exit 0' >> ~/wsl_guardian_agent.sh",
                "echo 'touch /tmp/wsl_guardian_active' >> ~/wsl_guardian_agent.sh",
                "echo 'while true; do' >> ~/wsl_guardian_agent.sh",
                "echo '  history -a' >> ~/wsl_guardian_agent.sh",
                "echo '  tail -n 1 ~/.bash_history >> \"$log_path\"' >> ~/wsl_guardian_agent.sh",
                "echo '  ps -eo pid,ppid,cmd | grep -E \"nc|bash -i|curl|wget|python|chmod|sh\" | grep -v grep >> \"$log_path\"' >> ~/wsl_guardian_agent.sh",
                "echo '  sleep 3' >> ~/wsl_guardian_agent.sh",
                "echo 'done' >> ~/wsl_guardian_agent.sh",
                "chmod +x ~/wsl_guardian_agent.sh",
            ]

            for line in lines:
                print(f"[COMMAND] wsl bash -c \"{line}\"")
                subprocess.run(["wsl", "bash", "-c", "echo 'export PROMPT_COMMAND=\"history -a; tail -n 1 ~/.bash_history >> /mnt/c/ProgramData/WSLGuardian/wsl_shell_trace.log\"' >> ~/.bashrc"])


            logger.info("WSL agent deployed and launched successfully.")

        except Exception as e:
            logger.error(f"Failed to deploy WSL agent: {e}")
    ...
    def terminate_wsl(self, pid, reason):
        ...
        try:
            ...
            if psutil.pid_exists(pid):
                os.kill(pid, signal.SIGKILL) #Signal terminates
    ...
    def check_for_unauthorized_commands(self, text):
        ...
        check for comman usage pattern
        """
